...
const AdminDashboard = () => {
  ...
  const loadDashboardData = useCallback(async () => {
    ...
    try {
      ...
      setPatients(
        patientsResult.status === "fulfilled" ? patientsResult.value.data : []
      );

      setDoctors(
        doctorsResult.status === "fulfilled" ? doctorsResult.value.data : []
      );

      setManagers(
        managersResult.status === "fulfilled" ? managersResult.value.data : []
      );
  ...
  const appointmentColumns = [
    {
      header: "Patient",
      cell: (appt) => safeRender(appt.patientUser?.username),
    },
    {
      header: "Doctor",
      cell: (appt) => `Dr. ${safeRender(appt.doctorUser?.username)}`,
    },
    {
      header: "Date & Time",
      cell: (appt) => safeDateTime(appt.appointmentDateTime),
    },
    {
      header: "Status",
      cell: (appt) => (
    ...
    { header: "Created", cell: (doc) => safeDate(doc.createdAt) },
    {
      ...
      cell: (doc) => (
        <div className="action-buttons">
          <button
            className="btn-toggle"
            onClick={() => handleToggleUserStatus(doc.userId)}
          >
          ...
          <button
            className="btn-reset"
            onClick={() => handleResetPassword(doc.userId)}
          >
            Reset Password
          </button>
        </div>
      ...
      <div className="overview-section">
        ...
        <div className="stats-grid">
          ...
          <div className="stat-card">
            ...
            <p className="stat-number">{managers?.length || 0}</p>
          </div>
          <div className="stat-card">
    ...
    <ErrorBoundary>
      <div className="users-section">
        <div className="content-header">
          ...
          <p>View and manage all system appointments</p>
        </div>
        {appointmentsError ? (
          <div className="error-message">{appointmentsError}</div>
        ) : (
          <PaginatedTable
            data={appointments}
