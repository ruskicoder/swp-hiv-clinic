...
% SVG package not available, using manual approach
\geometry{margin=1in}
\pagestyle{fancy}
\fancyhf{}
\rhead{\thepage}
...
\begin{figure}[H]
\centering
% Note: SVG file exists at diagrams/use_case_diagram.svg
% For LaTeX compilation, a PDF version would be needed
\fbox{\parbox{0.9\textwidth}{\centering\textbf{HIV Clinic Management System Use Case Diagram}\\[10pt]
\textit{27 Use Cases (UC-001 through UC-027)}\\[5pt]
\textit{Actors: Unauthenticated User, Authenticated User}\\[5pt]
\textit{SVG Source: diagrams/use\_case\_diagram.svg}}}
...
\begin{figure}[H]
    \centering
    % Note: SVG file exists at diagrams/user_interface_flow.svg
    \fbox{\parbox{0.9\textwidth}{\centering\textbf{Overall Screen Flow}\\[10pt]
    \textit{Role-based interface navigation}\\[5pt]
    \textit{Public → Authentication → Role-specific dashboards}\\[5pt]
    \textit{SVG Source: diagrams/user\_interface\_flow.svg}}}
